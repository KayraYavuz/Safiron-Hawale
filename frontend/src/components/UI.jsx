...
export function TrHover({ children, onClick, style={} }) {
  return (
    <tr
      onClick={onClick}
      style={{ transition:'background 0.1s', cursor: onClick ? 'pointer' : 'default', ...style }}
...
export function Badge({ type, children, dot=false }) {
  ...
  return (
    <span style={{
      ...
      {dot && <span style={{ width:5, height:5, borderRadius:'50%', background:th.dot, flexShrink:0 }} />}
      {children}
    </span>
  )
}
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:4 }}>
      {Array.from({ length:total }).map((_,i) => (
        <div key={i} style={{
          borderRadius:99, transition:'all 0.2s ease',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: color || C.navy, opacity:0.6,
      }} />
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:12 }}>
        ...
        {IconComp && (
          <div style={{
            ...
            background: color ? `${color}12` : C.surface3,
            display:'flex', alignItems:'center', justifyContent:'center',
          }}>
